...
function App() {
  ...
  // Optimized section classes generator
  const getSectionClasses = useCallback((sectionId, baseClasses = "") => {
    const isVisible = visibleSections.has(sectionId);
    const animationClasses = isVisible 
      ? "opacity-100 translate-y-0 scale-100" 
      : "opacity-0 translate-y-6 scale-98";
    
    return `${baseClasses} transition-all duration-700 ease-out transform ${animationClasses}`;
  }, [visibleSections]);
      ...
      <div className="relative z-[2]">
        {sections.map(({ id, Component, props }) => (
          <section
            ...
            id={id}
            ref={sectionRefs[id]}
            className={getSectionClasses(id, "min-h-screen")}
          >
            <Component {...props} />
          </section>
        ))}
