      ...
      <div className="relative z-10 min-h-screen flex flex-col justify-center px-6 md:px-12 py-24">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {contactMethods.map(method => (
            <div key={method.id} className={`p-6 rounded-lg border ${method.borderColor} transition-all duration-300`}>
              <div className="flex items-center mb-4">
                <span className="text-2xl">{method.icon}</span>
                <h3 className="font-orbitron text-xl font-bold ml-2">{method.title}</h3>
              </div>
              <p className="font-rajdhani text-lg">{method.primary}</p>
              <p className="font-rajdhani text-sm text-white/70">{method.secondary}</p>
            </div>
          ))}
        </div>
